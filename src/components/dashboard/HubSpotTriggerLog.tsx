...
/**
 * HubSpot Trigger Activity Log
 *
 * Displays recent HubSpot webhook trigger events with status,
...
import type { HubSpotTriggerLog as HubSpotTriggerLogType } from '@/types';

interface HubSpotTriggerLogProps {
    logs: HubSpotTriggerLogType[];
}
...
export function HubSpotTriggerLog({ logs }: HubSpotTriggerLogProps) {
    if (logs.length === 0) {
        return (
            <div className="text-center py-8 text-muted-foreground">
                <p className="text-sm">No trigger events yet</p>
                <p className="text-xs mt-1">Events will appear here when HubSpot workflows trigger outbound calls</p>
            </div>
        );
    }
